...
public class Blowfish {
    ...
    private String xOr(String lhs, String rhs) {
        StringBuilder result = new StringBuilder();

        for (int i = 0; i < lhs.length(); ++i) {
            // Do xOr operaton on all individual numbers in string
            // .charAt basically takes the char from whichever position j is at in the
            // strings
            // Numero uno
            char num1 = lhs.charAt(i);
            // Numero dos
            char num2 = rhs.charAt(i);
    ...
    private String addMod(String lhs, String rhs) {
        StringBuilder result = new StringBuilder();

        // add 2 binary strings with modulo 2^32,
        // then add that to the result string
    ...
    private String F(String input) {
        String HexStr = HexToBin(StrToHex(input));
        String[] SBox = new String[4];

        String temp = "";
        for (int i = 0; i < 8; i += 2) {
            temp = HexStr.substring(i, i + 2);
            SBox[i / 2] = S[i / 2][Integer.parseInt(temp, 16)];
        }

        return (
            addMod(
                xOr(
                    addMod(SBox[1], SBox[2]), 
                    SBox[3]), 
                SBox[4])
            );
    }
